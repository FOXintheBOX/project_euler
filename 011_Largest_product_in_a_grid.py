...
for col in range(len(grid) - patt_len + 1):
    for row in range(len(grid[0]) - patt_len + 1):
        horiz = 1
        vert = 1
        down_diag = 1
        up_diag = 1
        for i in range(patt_len):
            horiz *= grid[col][row + i]
            vert *= grid[col + i][row]
            down_diag *= grid[col + i][row + i]
            up_diag *= grid[col + patt_len - 1 - i][row + i]
        max_direc = max(horiz, vert, down_diag, up_diag)
        if max_direc > max_prod: max_prod = max_direc
